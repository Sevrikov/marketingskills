...
$runRoot = Join-Path $backend (".tmp\realistic-solar-category-runs\" + [guid]::NewGuid().ToString("N"))
...
try {
  & $python -m pytest tests/test_realistic_solar_category_smoke.py --basetemp $baseTemp -p no:cacheprovider
