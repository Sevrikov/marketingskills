...
$runRoot = Join-Path $backend (".tmp\realistic-solar-article-runs\" + [guid]::NewGuid().ToString("N"))
...
try {
  & $python -m pytest tests/test_realistic_solar_article_smoke.py --basetemp $baseTemp -p no:cacheprovider
